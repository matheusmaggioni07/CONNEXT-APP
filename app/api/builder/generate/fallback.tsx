...
function extractSiteName(prompt: string): string {
  ...
  if (lowerPrompt.includes("agro")) return "AgroBrasil"

  const words = prompt.split(" ")
  for (const word of words) {
    if (
      word.length > 3 &&
      !["site", "para", "criar", "crie", "faça", "faca", "quero", "um", "uma", "do", "da", "de", "com"].includes(
        word.toLowerCase(),
      )
    ) {
...
function getTeamColors(prompt: string): { primary: string; secondary: string; gradient: string } {
  ...
  if (lowerPrompt.includes("agro") || lowerPrompt.includes("fazenda")) {
    return { primary: "#16A34A", secondary: "#15803D", gradient: "from-green-600 to-green-800" }
  }
...
export function generateFallbackCode(prompt: string): string {
  ...
  // Build the code as a simple string without complex interpolation
  const gradientClass = colors.gradient
  const firstLetter = siteName.charAt(0)
  const sectionTitle = isSports ? "Time" : "Serviços"
  const heroText = isSports
  ...
  const stat1Label = isSports ? "Anos de História" : "Anos de Mercado"
  const stat2Label = isSports ? "Torcedores" : "Clientes"
  const stat3Label = isSports ? "Títulos" : "Projetos"
  const stat1Value = isSports ? "100+" : "10+"
  const stat2Value = isSports ? "40M+" : "5K+"
  const stat3Value = isSports ? "30+" : "100+"
  const feature1Title = isSports ? "Garra e Determinação" : "Agilidade"
  const feature1Text = isSports ? "Nosso time luta até o último minuto." : "Entregamos resultados rápidos."
  const feature2Title = isSports ? "Tradição e Conquistas" : "Qualidade"
  const feature2Text = isSports ? "Uma história repleta de títulos." : "Comprometimento com excelência."
  const feature3Title = isSports ? "Torcida Apaixonada" : "Suporte Dedicado"
  const feature3Text = isSports ? "Milhões de corações que batem juntos." : "Equipe sempre disponível."
  const footerText = isSports
    ? `A paixão que nos une. O ${siteName} é mais que um clube!`
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br ${gradientClass} text-white">
      <nav className="fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-sm border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br ${gradientClass} flex items-center justify-center font-bold text-xl">${firstLetter}</div>
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <button onClick={() => scrollToSection('features')} className="text-sm font-medium hover:text-gray-300 transition-colors">${sectionTitle}</button>
        ...
        {menuOpen && (
          <div className="md:hidden bg-black/95 border-t border-white/10">
            <div className="px-4 py-4 space-y-3">
              ...
              <button onClick={() => scrollToSection('features')} className="block w-full text-left px-4 py-2 rounded-lg hover:bg-white/10 transition-colors">${sectionTitle}</button>
      ...
      <section id="hero" className="pt-16 min-h-screen flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          ...
          <h1 className="text-5xl md:text-7xl font-bold mb-6 leading-tight">${siteName}</h1>
          <p className="text-xl md:text-2xl text-gray-300 mb-10 max-w-2xl mx-auto">${heroText}</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button onClick={() => scrollToSection('sobre')} className="px-8 py-4 bg-white text-black font-semibold rounded-full hover:bg-gray-200 transition-all transform hover:scale-105 shadow-xl">Saiba Mais</button>
            <button onClick={() => scrollToSection('contato')} className="px-8 py-4 bg-transparent border-2 border-white font-semibold rounded-full hover:bg-white hover:text-black transition-all transform hover:scale-105">Entre em Contato</button>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <p className="text-lg text-gray-300 leading-relaxed">${aboutText1}</p>
              <p className="text-lg text-gray-300 leading-relaxed">${aboutText2}</p>
              <div className="grid grid-cols-3 gap-4 pt-6">
                <div className="text-center p-4 rounded-xl bg-white/5 backdrop-blur-sm">
                  <div className="text-3xl font-bold">${stat1Value}</div>
                ...
                <div className="text-center p-4 rounded-xl bg-white/5 backdrop-blur-sm">
                  <div className="text-3xl font-bold">${stat3Value}</div>
                  <div className="text-sm text-gray-400">${stat3Label}</div>
                </div>
              </div>
            </div>
            <div className="relative">
              <div className="aspect-square rounded-2xl bg-gradient-to-br ${gradientClass} p-1">
                <div className="w-full h-full rounded-xl bg-black/50 flex items-center justify-center">
                  <div className="text-8xl font-bold opacity-50">${firstLetter}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="features" className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-sm font-semibold tracking-wider uppercase text-gray-400">${sectionTitle}</span>
            <h2 className="text-4xl md:text-5xl font-bold mt-2">Destaques</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="group p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/30 transition-all hover:-translate-y-2">
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br ${gradientClass} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
              </div>
              <h3 className="text-xl font-bold mb-3">${feature1Title}</h3>
              <p className="text-gray-400">${feature1Text}</p>
            </div>
            <div className="group p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/30 transition-all hover:-translate-y-2">
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br ${gradientClass} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              </div>
              <h3 className="text-xl font-bold mb-3">${feature2Title}</h3>
              <p className="text-gray-400">${feature2Text}</p>
            </div>
            <div className="group p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/30 transition-all hover:-translate-y-2">
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br ${gradientClass} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              </div>
              <h3 className="text-xl font-bold mb-3">${feature3Title}</h3>
              <p className="text-gray-400">${feature3Text}</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl focus:outline-none focus:border-white/40 transition-colors text-white" placeholder="Seu nome" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Email</label>
                <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl focus:outline-none focus:border-white/40 transition-colors text-white" placeholder="seu@email.com" required />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Mensagem</label>
              <textarea value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} rows={5} className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl focus:outline-none focus:border-white/40 transition-colors resize-none text-white" placeholder="Sua mensagem..." required></textarea>
            </div>
            <button type="submit" className="w-full py-4 bg-gradient-to-r ${gradientClass} font-semibold rounded-xl hover:opacity-90 transition-opacity transform hover:scale-[1.02] active:scale-[0.98]">Enviar Mensagem</button>
          </form>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-4 gap-8">
            <div className="md:col-span-2">
              ...
              <p className="text-gray-400 max-w-md">${footerText}</p>
            </div>
            <div>
              ...
              <ul className="space-y-2 text-gray-400">
                ...
                <li><button onClick={() => scrollToSection('features')} className="hover:text-white transition-colors">${sectionTitle}</button></li>
